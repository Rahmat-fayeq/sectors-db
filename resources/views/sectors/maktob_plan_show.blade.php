	...
	<div class="row">
      <div class="col-sm-12">
          <div class="card-box table-responsive" style="box-shadow: 1px 2px 12px 12px silver">
              <a href="{{ route('sector.maktob_plan_add') }}" class="btn btn-primary btn-md waves-effect waves-light m-b-3"
                    >
                <i class="md md-add"></i> ثبت کردن معلومات
              </a>   
              <div class="dropdown pull-right">
                <a href="{{ route('sector.maktob_plan_excel') }}" class="btn btn-success m-b-3">
                  <i class="glyphicon glyphicon-download"></i>
                </a> 
              </div>     
              <h4 class="header-title m-t-0 m-b-30" style="text-align:center;">مکاتیب پلان</h4>
              <table id="datatable" class="table table-bordered  table-hover">
                ...
                <tr style="background-color:#435966;">
                  ...
                  <th style="text-align:center;color:#fff;"> مکتوب پلان</th>
                  <th style="text-align:center;color:#fff;"> تاریخ مکتوب پلان</th>
                  <th style="text-align:center;color:#fff;">حالت پلان</th>
                  <th style="text-align:center;color:#fff;">مکتوب جواب</th>
                  <th style="text-align:center;color:#fff;">تاریخ جواب</th>
                  <th style="text-align:center;color:#fff;">خلاصه موضوع</th>
              ...
              <tr>
                ...
                <td>{{ $data->rmaktob_no }}</td>
                <td>{{ $data->rmaktob_date }}</td>
                <td>{{ $data->rmaktob_subject	 }}</td>
        				<td>{{ $data->doc_no }}</td>
                <td>{{ $data->shelf_no	 }}</td>
                ...
                <td>
                  @if(!empty($data->file))
                    <span>
                      <a href="{{ route('sector.plan_download', $data->file) }}" class="btn btn-primary">
                ...
                <span style="margin-right: 7%;">
                      <a href="{{ route('sector.maktob_plan_edit',$data->id) }}" class="btn btn-default">
                ...
                <td>
                <span style="margin-right: 7%;">
                  <a onclick="return confirm('آیا مطمئین هستید که حذف شود؟')" href="{{ route('sector.maktob_plan_delete',$data->id) }}" class="btn btn-default">
